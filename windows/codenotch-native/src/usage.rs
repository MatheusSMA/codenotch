...
//! Ported from the Tauri build's `usage.rs`. The fetching, the credential handling, the back-off
//! and the token renewal are untouched — they encode a lot of hard-won behaviour about how this
//! endpoint answers, and any drift would be a bug. What changed is the three lines that were
//! Tauri: shared state comes from a `Hub`, and a change raises a flag the render loop reads
//! instead of emitting to a WebView. It still writes `usage.json`, which is what the pill reads.
// Kept as they came across: request_refresh and probe_credentials serve the upstream settings window, and pruning them would be drift from upstream for no gain.
#![allow(dead_code)]
...
fn store_path() -> std::path::PathBuf {
    crate::data_dir().join("usage.json")
}
...
impl Renewer {
    ...
    fn maybe_renew(&mut self, cred: &Credential) -> Option<bool> {
        ...
        let Some(cli) = find_cli() else {
            crate::trace("claude: token about to expire and no standalone claude CLI found to renew it");
            return Some(false);
        };
        if let Err(e) = run_renewal(&cli) {
            crate::trace(&format!("claude: token renewal could not start ({}): {e}", cli.display()));
            return Some(false);
        }
        let after = read_credentials().and_then(|c| c.expires_at);
        let renewed = matches!((after, cred.expires_at), (Some(a), Some(b)) if a > b);
        crate::trace(&if renewed {
...
fn set_and_broadcast(hub: &Hub, mutate: impl FnOnce(&mut UsageSnapshot)) {
    let snap = {
        let mut u = hub.usage.lock().unwrap();
        mutate(&mut u);
        u.clone()
    };
    persist(&snap);
...
pub fn start(hub: Arc<Hub>) {
    std::thread::spawn(move || {
        // Show the persisted old reading at startup: stale beats blank
        hub.mark_changed();
        ...
        loop {
            ...
            if let Some(cred) = read_credentials() {
                if renewer.maybe_renew(&cred) == Some(true) {
                    consecutive_429 = 0;
                    set_and_broadcast(&hub, |u| u.backoff_until = 0);
                }
            }
            // No requests inside the backoff window
            let bu = {
                let u = hub.usage.lock().unwrap();
                u.backoff_until
            };
            let now = now_ms();
            if bu > now {
            ...
            match read_credentials() {
                ...
                Some(cred) if cred.expired(now_ms()) => set_and_broadcast(&hub, |u| {
                ...
                Some(cred) => {
                    ...
                    match result {
                        ...
                        Err(FetchErr::NeedsAuth) => set_and_broadcast(&hub, |u| {
                            u.status = "needsAuth".into();
                        ...
                        Err(FetchErr::Other(msg)) => set_and_broadcast(&hub, |u| {
                            if u.windows.is_empty() {
                                u.status = "error".into();
                            } else {
                                u.status = "stale".into();
