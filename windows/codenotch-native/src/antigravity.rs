//! Ported from the Tauri build (`windows/codenotch/src`) with the AppHandle swapped for the Hub.
...
fn store_path() -> PathBuf {
    crate::data_dir().join("antigravity.json")
}
...
fn read_once(rt: &mut Runtime, prev: &UsageSnapshot) -> UsageSnapshot {
    ...
    if tried && !bridge_err.is_empty() {
        crate::trace(&format!("antigravity: local bridge failed ({bridge_err})"));
    }
    ...
    match read_credentials() {
        Some(c) if !c.expired => match load_tier(&c.access_token) {
            ...
            Err(e) => crate::trace(&format!("antigravity: loadCodeAssist {e}")),
        },
        Some(c) => {
...
fn broadcast(hub: &Hub, snap: UsageSnapshot) {
    *hub.antigravity.lock().unwrap() = snap.clone();
    persist(&snap);
    hub.mark_changed();
}
    ...
    *REFRESH_CLI.lock().unwrap() = Some(sender);
    ...
    std::thread::spawn(move || {
        ...
        while receiver.recv().is_ok() {
            ...
            let previous = hub.antigravity.lock().unwrap().clone();
...
fn start_legacy(hub: Arc<Hub>) {
    std::thread::spawn(move || {
        ...
        loop {
            let prev = {
                let s = hub.antigravity.lock().unwrap().clone();
                s
            };
