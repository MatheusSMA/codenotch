...
//! Ported from the Tauri build's `codex.rs`, with the same three Tauri lines swapped for a `Hub`
//! as in `usage.rs`. The reading logic is untouched.
//!
//! Upstream this file borrowed one base64 decoder from `antigravity.rs`. That module is 35 KB for
//! a provider this build does not carry, so the decoder is copied in below rather than dragging
//! the rest of it along.
// Kept as they came across: find_executable, probe and friends serve the upstream settings window, and pruning them would be drift from upstream for no gain.
#![allow(dead_code)]
...
fn store_path() -> PathBuf {
    crate::data_dir().join("codex.json")
}
    ...
    match resp {
        ...
        Err(ureq::Error::Status(code @ (401 | 403), r)) => {
            ...
            let head: String = r
                ...
                .take(160)
                .collect();
            crate::trace(&format!("codex: usage endpoint HTTP {code}: {head}"));
        ...
        match load_credential() {
            None => {
                if auth_path().map(|p| p.is_file()).unwrap_or(false) {
                    crate::trace("codex: auth.json has no usable access_token/account_id, falling back to the rollout");
                }
            }
            Some(cred) => match fetch_usage(&cred) {
                Ok(v) => {
                    ...
                    crate::trace(&format!("codex: usage reply has no windows (top-level keys {keys:?}), falling back to the rollout"));
                ...
                Err(LiveErr::RateLimited(secs)) => {
                    ...
                    crate::trace(&format!("codex: usage endpoint returned 429, retrying in {secs}s"));
                }
                Err(LiveErr::Other(e)) => {
                    crate::trace(&format!("codex: live read failed ({e}), falling back to the rollout"));
...
fn broadcast(hub: &Hub, snap: UsageSnapshot) {
    *hub.codex.lock().unwrap() = snap.clone();
    persist(&snap);
    hub.mark_changed();
}
...
pub fn start(hub: Arc<Hub>) {
    std::thread::spawn(move || {
        ...
        hub.mark_changed();
        if !present() {
            broadcast(&hub, UsageSnapshot { status: "absent".into(), ..Default::default() });
