...
//!
//! Taken from the Tauri build unchanged. Without these entries nothing ever posts to the hook port,
//! and `attention` â€” the one state that cannot be inferred from disk â€” never fires. On this machine
//! they had never been installed, so the amber ring could only be produced by hand.

// uninstall is unused today but is the other half of install: dropping it would leave no way
// to take these entries back out.
#![allow(dead_code)]
